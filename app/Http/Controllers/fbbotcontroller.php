...
class fbbotcontroller extends Controller
{
 public function callback(Request $request){
	...
	private function marketPaidPlans($recipientId, $messageText){
		...
		Cache::put('fb_user_id', $recipientId, 25);
		file_put_contents( "php://stderr","$recipientId");
		// $this->sendAction($recipientId);
    	// $user = $this->getUserDetails($recipientId);
		// $userdata = json_decode($user);
		$url = 'https://graph.facebook.com/v3.2/me/messages?access_token=' . env("PAGE_ACCESS_TOKEN");
                        ...
                        "message": {
                            "attachment": {
                                ...
                                "payload": {
                                    ...
                                    "elements": [
                                        {
                                        "title":"Buy our plan for 50 markets subscriptions",
                                        "subtitle":"You need to pay $50",
                                            "buttons":[
                                                {
                                               "type":"web_url",
                                        ...
                                        {
                                        "title":"Buy our plan for 100 markets subscriptions",
                                        "subtitle":"You need to pay $100",
                                            "buttons":[
                                                {
                                               "type":"web_url",
												"url":"http://crypto-chat-bot.herokuapp.com/payment",
